...
class Mouse:
    ...
    def __init__(self, mouse_i: int, *, img_num: int, is_test: bool, preprocess: bool = False,
                 data_dir: str = '../data/mouse%d/', prefix: List[str] = None):
        """
        ...
        # Read data
        print("Reading mouse%d ..." % (mouse_i + 1))
        time_start = time.time()
        file_path = data_dir % (mouse_i + 1)
        self._o_images = []
        self._g_images = []
        for aug_i in range(img_num):
            name = [file_path + prefix[ii] + '%d.tiff' % aug_i for ii in [0, 1]]
            print('  ' + name[0])

            # read data
            filename = '../data/mouse%d/f0.mat'%(mouse_i+1)
            mat_contents = sio.loadmat(filename)
            temp_o = mat_contents['cube'] #arrangement is (x,y,z) in order

            #temp_o = np.stack(
            #    cv2.imreadmulti(name[0], flags=0)[1],
            #    axis=2)

            if preprocess:
                temp_o_o_o = 1 #do nothing
            ...
            print('  ' + name[1])
            temp_g = np.stack(
                cv2.imreadmulti(name[1], flags=0)[1],
                axis=2)
            temp_g[temp_g > 0] = 1
            self._set_or_check_shape(temp_g)
            self._g_images.append(temp_g)
        print("Reading mouse%d finish, time %f s" % (mouse_i + 1, time.time() - time_start))
