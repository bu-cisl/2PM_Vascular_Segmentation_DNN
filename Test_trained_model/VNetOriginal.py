...
def _convolution_block(layer_input, n_channels, num_convolutions):
    x = layer_input
...
def _convolution_block_2(layer_input, fine_grained_features, n_channels, num_convolutions):
...
def v_net(tf_input, input_channels, output_channels=1, n_channels=16, name=None):
    ...
    Returns:
        ...
        dropout.
    """
    with tf.variable_scope(name, 'VNet'):
        ...
        with tf.variable_scope('expanding_path'):
            ...
            with tf.variable_scope('level_3'):
                e3 = _convolution_block_2(e42, c3, n_channels * 4, 3)
                e32 = _up_convolution(e3, tf.shape(c2), n_channels * 4)

            with tf.variable_scope('level_2'):
                e2 = _convolution_block_2(e32, c2, n_channels * 2, 2)
                e22 = _up_convolution(e2, tf.shape(c1), n_channels * 2)

            with tf.variable_scope('level_1'):
                e1 = _convolution_block_2(e22, c1, n_channels, 1)
